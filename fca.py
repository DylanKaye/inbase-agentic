...
def fca(base, seat, d1, d2, seconds):
    ...
    try:
        ...
        if len(c_idxs) > 0:
            ...
            char_val = 0
        objective = cp.Maximize(.15*cp.sum(cdos) - .2*cp.sum(chnk) + 4*cp.sum(cp.multiply(po,sen)) + 2*cp.sum(cp.multiply(pover,sen)) + .2*cp.sum(cp.multiply(ptime,sen)) + 2.5*res_val + char_val)
