...
def fca(base, seat, d1, d2, seconds):
    ...
    try:
        ...
        max_time_distance = 20  # Maximum hours distance in a day
        ...
        for c in range(n_c):
            ...
            # Reserve bonuses for those who prefer reserves
            if len(r_idxs) > 0 and pref_reserves[c] == 0:  # Doesn'tPrefers reserves
                # Boost reserve bonuses
                for idx in r_idxs:
                    bonuses[idx] = -5
                
            if len(r_idxs) > 0 and pref_reserves[c] == 2: 
                # Boost reserve bonuses
                for idx in r_idxs:
                    bonuses[idx] = 0
        ...
        if len(c_idxs) > 0:
            ...
            char_val = 0
        objective = cp.Maximize(.15*cp.sum(cdos) - .25*cp.sum(chnk) + 10*cp.sum(cp.multiply(po,sen*10)) + 5*cp.sum(cp.multiply(pover,(sen**2)*20)) + 1*cp.sum(cp.multiply(excov,sen*20)) + 1*cp.sum(cp.multiply(ptime,sen*5)) + 5*res_val + char_val)
